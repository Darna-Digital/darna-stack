...
const run = <Success, Failure>(
  ...
  Effect.runPromise(
    effect.pipe(Effect.either, Effect.provide(ProjectsMemory(options?.seed))),
  )
...
describe("Projects.update", () => {
  it("merges patch fields", async () => {
    const result = await run(
      Projects.update(seedProject.id, { name: "renamed" }),
      { seed: [seedProject] },
    )
    expect(Either.isRight(result)).toBe(true)
...
describe("Projects.remove", () => {
  ...
  it("fails with ProjectNotFound when the id is unknown", async () => {
    const result = await run(
      Projects.remove("88888888-8888-4888-8888-888888888888" as ProjectId),
    )
    expect(Either.isLeft(result)).toBe(true)
    if (Either.isLeft(result))
      expect(result.left._tag).toBe("ProjectNotFound")
