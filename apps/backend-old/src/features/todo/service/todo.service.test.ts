...
) => Effect.runPromise(effect.pipe(Effect.either, Effect.provide(TodosMemory(options?.seed))));
...
describe("Todos.update", () => {
  it("merges patch fields", async () => {
    const result = await run(Todos.update(seedTodo.id, { done: true, title: "renamed" }), {
      seed: [seedTodo],
    });
    expect(Either.isRight(result)).toBe(true);
    if (Either.isRight(result)) {
  ...
  it("fails with TodoNotFound for an unknown id", async () => {
    ...
    expect(Either.isLeft(result)).toBe(true);
    if (Either.isLeft(result)) expect(result.left._tag).toBe("TodoNotFound");
  });
});
...
describe("Todos.remove", () => {
  ...
  it("fails with TodoNotFound when the id is unknown", async () => {
    const result = await run(Todos.remove("88888888-8888-4888-8888-888888888888" as TodoId));
    expect(Either.isLeft(result)).toBe(true);
    if (Either.isLeft(result)) expect(result.left._tag).toBe("TodoNotFound");
  });
});
